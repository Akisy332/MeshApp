...
from .element import Tile, Marker, Buttons, Path, SimplePath
...
class MapView(QGraphicsView):
    def __init__(self, *args,
                 ...
                 buttons: bool = True,
                 homeCoord: tuple[float, float] = None,
                 **kwargs):
        ...
        if homeCoord is None:
            self.homeCoord = (56.45112740752376, 84.96449640447315)
    ...
    def setTileServer(self, tileServer: str, useDatabaseOnly: bool = False, dataPath: str | None = None):
        ...
        if not self.init is True:
            ...
            self.tileManager = TileManager(self, self.useDatabaseOnly, dataPath)
            self.tileManager.signalUpdateTile.connect(self.__updateTile)
    
    def getTileServer(self) -> dict:
        """ Returns the current tile server """
        return self.mapLayers[self.currentLayers]
    ...
    def setPosition(self, position: tuple = None):
        ...
        if position is None or type(position) is bool:
            position = self.homeCoord
    ...
    def __updateTile(self, data: tuple):
        data[0].setImage(data[1])
    ...
    def __drawMove(self, called_after_zoom: bool = False):
        ...
        # Если прошло больше 1 / 120 секунды, обновляем FPS
        if elapsed_time >= 1.0 / self.maxFps or called_after_zoom:
            ...
            if self.canvas_tile_array:
                ...
                self.drawTileCount+=1
                if time.time() - self.fpsTime >= 1.0:  
                    fps = self.frame_count / (time.time() - self.fpsTime)
                    self.fpsTime = time.time()
                    # print(f"FPS: {fps:.2f}")
                    # print(f"Draw: {self.drawTileCount:.2f}")
                    self.frame_count = 0    
                    self.drawTileCount = 0   
    ...
    def resizeEvent(self, event):
        ...
        if not self.buttons.buttonHome is None:
            self.buttons.buttonHome.setGeometry(self.size().width() - 55, self.size().height()-55, 35, 35)
    ...
    def contextMenuEvent(self, event):
        ...
        for command in self.right_click_menu_commands:
            ...
            if command["pass_coords"]:
                m.addAction(command["label"], 
                           lambda coord=coordinate_mouse_pos, cmd=command: cmd["command"](coord))
            else:
...
class TileManager(QObject):
    signalUpdateTile = pyqtSignal(tuple)
    def __init__(self, gui: "MapView", useDatabaseOnly: bool, dataPath: str):
        super().__init__()
        self.gui = gui
        self.useDatabaseOnly = useDatabaseOnly
    ...
    def setDataPath(self, dataPath: str, dataBase: bool):
        ...
        self.dataPath = dataPath + ".db" if dataBase else dataPath
        # print("map:", self.dataPath)
        ...
        if self.preCacheThread is not None and self.preCacheThread.is_alive():
            self.preCacheThread.join(timeout=1.0)
            self.preCacheThread = threading.Thread(daemon=True, target=self.preCache)
        
        self.stop_image_load_threads()
        ...
        for thread in self.imageLoadThreadPool:
            thread.start()
       
    def stop_image_load_threads(self):
        # 1. Остановить все текущие потоки (если они есть)
        for thread in self.imageLoadThreadPool:
            if thread.is_alive():
                thread.join(timeout=1.0)
    
        # 2. Очистить пул
        self.imageLoadThreadPool.clear()

        # 3. Создать и запустить новые потоки
        for _ in range(25):
            new_thread = threading.Thread(daemon=True, target=self.loadImagesBackground)
            self.imageLoadThreadPool.append(new_thread)
    ...
    def preCache(self):
        ...
        # print("Map db path:", self.dataPath)
    ...
    def updateTileImages(self):
        while len(self.imageLoadQueueResults) > 0 and self.running:
            ...
            if zoom == round(self.gui.zoom):
                self.signalUpdateTile.emit((tile, image))
